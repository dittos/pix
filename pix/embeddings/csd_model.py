# Original code from https://github.com/learn2phoenix/CSD
# Modified to use OpenCLIP
...
class CSD_CLIP(nn.Module):
    ...
    def __init__(self, name='vit_large',content_proj_head='default'):
        ...
        if name == 'vit_large':
            ...
            clipmodel = open_clip.load_openai_model("ViT-L-14", device="cpu")
            self.backbone = clipmodel.visual
            self.embedding_dim = 1024
        elif name == 'vit_base':
            # clipmodel, _ = clip.load("ViT-B/16")
            clipmodel = open_clip.load_openai_model("ViT-B-16", device="cpu")
            self.backbone = clipmodel.visual
